...
--[[ Repo: https://github.com/SinTan1729/koreader-patches
    ...
    https://github.com/zenixlabs/koreader-frankenpatches-public/blob/main/2-reading-insights-popup.lua
...
local function formatHoursRead(seconds)
    ...
    if h == 0 then
        ...
        return tostring(h), 'hours read'
    end

    return tostring(h), h_unit
end
...
local function buildMonthlyChart(popup_self, monthly_data, layout, fonts)
    ...
    for i = 1, #monthly_data, 6 do
        ...
        if #row_data > 0 then
            ...
            --FocusManager
            for _, month_num in ipairs(nonZeroMonths) do
                if month_num <= 6 then
...
local function buildBestStreakWidget(streaks, streaks_dimen, fonts, streaks_colors)
    local function buildBestModule(value, weekOrDay, isLongest, ts_start, ts_end)
        ...
        local value_text = weekOrDay == 0 and N_('week', 'weeks', streaks.weeks.best) or
            N_('day', 'days', streaks.days.best)
...
local function buildInsightsSections(popup_self, streaks, yearly_stats, yearRange, monthly_data, fonts, layout, _)
    --FocusManager
    popup_self.layout = {}
...
local function showBooksForPeriod(_, books, empty_text, title)
    if #books == 0 then
        InfoMessage = require('ui/widget/infomessage')
        UIManager:show(InfoMessage:new {
            text = empty_text,
...
function ReadingInsightsPopup:showBooksForMonth(year_month, month_label_full)
    ...
    showBooksForPeriod(
        ...
        T(('%1 - %2 ' .. N_('book', 'books', bookCount) .. ' (%3 ' .. N_('page', 'pages', pages) .. ')'),
            month_label_full, bookCount, pages)
    )
end
...
function ReadingInsightsPopup:showBooksForYear(year)
    ...
    showBooksForPeriod(
        ...
        T(('%1 - %2 ' .. N_('book', 'books', bookCount) .. ' (%3 ' .. N_('page', 'pages', pages) .. ')'), year, bookCount,
            pages)
    )
end
...
local function populateEverything(popup_self, year, yearRange)
    ...
    local a = {
        ...
        monthlyReadingDays = insightsCache.monthlyReadingDays and insightsCache.monthlyReadingDays[year] or
            popup_self:getMonthlyReadingDays(year),
        monthlyReadingHours = insightsCache.monthlyReadingHours and insightsCache.monthlyReadingHours[year] or
            popup_self:getMonthlyReadingHours(year),
    }
    return a
end
...
function ReadingInsightsPopup:onSwipe(_, ges_ev)
...
function ReadingInsightsPopup:onTapClose(_, ges)
...
function FileManager:onShowReadingInsightsPopup(this)
    local popup = ReadingInsightsPopup:new {
        ui = this,
    }
    UIManager:show(popup)
