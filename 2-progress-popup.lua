...
--[[ Repo: https://github.com/SinTan1729/koreader-patches
    ...
    https://github.com/zenixlabs/koreader-frankenpatches-public/blob/main/2-cvs-receipt-frankenpatch.lua
]]
...
if g_reader_settings and g_reader_settings.isTrue then
    buttonProgressBarEnabled = g_reader_settings:isTrue('progress_popup_button_progbars')
    showBookCompleteWindow = g_reader_settings:isTrue('progress_popup_book_complete_window')
    altFontEnabled = g_reader_settings:isTrue('progress_popup_altFont')
    brtAuthorsEnabled = g_reader_settings:isTrue('progress_popup_brtAuthors')
end

-- ADD TO MENU

local prgMenu = {
    text = _('Progress Popup'),
    sorting_hint = 'tools',
    sub_item_table = {
        {
            text = _('button style progress bars'),
            help_text = _(
                'button style progress bars inspired by old kindle firmwares. when unchecked, the receipt goes back to regular progress bars.'),
            checked_func = function()
                return g_reader_settings:isTrue('progress_popup_button_progbars')
            end,
            callback = function()
                g_reader_settings:flipNilOrFalse('progress_popup_button_progbars')
                buttonProgressBarEnabled = g_reader_settings:isTrue('progress_popup_button_progbars')
            end,
        },
        {
            ...
            help_text = _(
                "a little window that pops up when you flip past the last page of a book. shows time read, starting date and highlight count for current book.\n\n(set Menu>gear icon>End of document action to 'Do nothing' for best results)."),
            checked_func = function()
                return g_reader_settings:isTrue('progress_popup_book_complete_window')
            end,
            callback = function()
                g_reader_settings:flipNilOrFalse('progress_popup_book_complete_window')
                showBookCompleteWindow = g_reader_settings:isTrue('progress_popup_book_complete_window')
            end,
        },
        {
            ...
            checked_func = function()
                return g_reader_settings:isTrue('progress_popup_brtAuthors')
            end,
            callback = function()
                g_reader_settings:flipNilOrFalse('progress_popup_brtAuthors')
                brtAuthorsEnabled = g_reader_settings:isTrue('progress_popup_brtAuthors')
            end,
        },
        {
            ...
            checked_func = function()
                return g_reader_settings:isTrue('progress_popup_altFont')
            end,
            callback = function()
                g_reader_settings:flipNilOrFalse('progress_popup_altFont')
                altFontEnabled = g_reader_settings:isTrue('progress_popup_altFont')
            end,
        },
    },
...
if not ReaderFooter._prg_popup_hooked then
    local orig_ReaderFooter_addToMainMenu_prg = ReaderFooter.addToMainMenu
    ReaderFooter.addToMainMenu = function(self, menu_items)
        if orig_ReaderFooter_addToMainMenu_prg then
            orig_ReaderFooter_addToMainMenu_prg(self, menu_items)
        end
        menu_items.progress_popup_menu = prgMenu
    end
    ReaderFooter._prg_popup_hooked = true
end
...
function quicklookwindow:init()
    ...
    local t = os.date('*t', ts_now) --[[@as osdate]]
    t.hour = 0
    t.min = 0
    t.sec = 0
    ...
    local function buildQuickLookWindow()
        ...
        local function itemname(book_or_ch_name)
            local title = tostring(book_or_ch_name)
            local widget = textboxx(title, w_font.face.reg, w_font.size.med, w_font.color.black, w_width, false, 'left')
            return widget
        end

        -- PROGRESS MODULE

        local function progressmodule(pgturn, pgturn_total, st_pageno, st_pagetotal) -- last two args rep. stable pages.
            ...
            local function buttonProgressBar(pct)
                ...
                if altFontEnabled then
                    ...
                    for _ = 1, repeatingUnit_reqNum do
        ...
        local time_read_today_box = function()
            local text = string.format('%s · %s', pagesReadToday_str, timeReadToday_str)
            local widget = textt(text, w_font.face.it, w_font.size.small, w_font.color.darkGray)
        ...
        local function time_left_display(timeleftstring, book_or_ch)
    ...
    local function buildBookCompleteWindow()
        ...
        local t_bookStart = os.date('*t', tonumber(ts_bookStart)) --[[@as osdate]]
        t_bookStart.hour = 0
        t_bookStart.min = 0
        t_bookStart.sec = 0
        ...
        local bookStartDate = ''
        if statsEnabled then
            bookStartDate = tostring(os.date('%d-%m-%Y', tonumber(ts_bookStart)))
        end
        ...
        local bookReadTime, highlightCount = 0, 0 -- bookreadtime is from FIRST OPEN till NOW
        if statsEnabled and bookCompleted then
            local _, time_placeholder = ReaderStatistics:getPageTimeTotalStats(ReaderStatistics
                .id_curr_book)
    ...
    local function buildBooksReadTodayWindow()
            ...
            for i = 1, #booksReadToday[1] do
                ...
                if brtAuthorsEnabled then                                  -- if authors enabled, then replaces "title" in [1][i] with "title - author(s)"
                    ...
                    if ReaderStatistics:getBookStat(bookId) then
                        ...
                        if auth ~= 'N/A' then auth_str = string.format(' - %s', string(auth)) end
        ...
        if statsEnabled and booksReadToday then
            ...
            for i = 1, #booksReadToday[1] do
                local text = tostring(booksReadToday[1][i]) -- book title
                local statsStr = booksReadToday[4][i]
                booksReadTodayWindow[#booksReadTodayWindow + 1] = booksReadTodayEntry(text, statsStr)
...
function quicklookwindow:onSwipe(_, ges_ev)
